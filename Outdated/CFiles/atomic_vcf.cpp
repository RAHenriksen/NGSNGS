...
void* Fafq_thread_se_run(void *arg){
  ...
  char region[128];
  snprintf(region, sizeof(region), "%s:%d-%d", chrID,74200,75000);
  fprintf(stderr,"Chromosomal region from vcf -> %s \t chromosome: %s\n",region,struct_obj->names[0]);
  
  hts_itr_t *itr = bcf_itr_querys(struct_obj->vcf_idx, struct_obj->vcf_hdr, region);
...
// g++ SimulAncient_func.cpp atomic.cpp -std=c++11 -I /home/wql443/scratch/htslib/ /home/wql443/scratch/htslib/libhts.a -lpthread -lz -lbz2 -llzma -lcurl -o bout
